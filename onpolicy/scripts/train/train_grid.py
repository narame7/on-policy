    ...
    def get_env_fn(rank):
        def init_env():
            if all_args.env_name == "Checkers-v0":
                ...
                env.share_observation_space = env.observation_space.copy()
                for i in range(len(env.observation_space)):
                    env.share_observation_space[i] = Box(
                        np.zeros(env.observation_space[i].low.shape[0] * 2),
                        np.ones(env.observation_space[i].high.shape[0] * 2),
                        (98,),
                        dtype=np.float32,
                    )
            else:
